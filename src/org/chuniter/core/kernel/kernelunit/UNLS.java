 ...
 * @since 3.1
 */
public abstract class UNLS {
	// 设置资源文件的类路径。
	private static final String BUNDLE_NAME = "org.chuniter.core.kernel.kernelunit.UNLS";// 定义KEY，与资源文件一致
	//通用属性
	/*** 创建人 **/
	...
	/*** 最后修改人 **/
	public static String lastModifyMan;
	/*** 最后修改日期 **/
	...
	/*** 审核人 **/
	public static String checkMan;
	/*** 备注 **/
	public static String remark;
	static {
		UNLS.initializeMessages(BUNDLE_NAME, UNLS.class);
	}
	...
	protected static final Map<String,Integer> hashChangePro = new HashMap<String,Integer>();
	...
	private static String[] buildVariants(String root) {
		Object l = null;
		if(null != AbstractBaseAuthorization.get())
			l = AbstractBaseAuthorization.get().getSession().getAttribute("language");
		if(null != l)
			chageLocale((String)l,AbstractBaseAuthorization.get().getSession().getId());
		if (nlSuffixes == null) {
	...
	public static void chageLocale(Locale l){ 
		if(null == l)
			l = Locale.getDefault();
		//build list of suffixes for loading resource bundles
		String nl = l.toString();
		List<String> result = new ArrayList<String>(4);
		int lastSeparator;
		while (true) {
			result.add('_' + nl + EXTENSION);
	...
	public static void chageLocale(String nl,String key){ 
		if(null == nl)
		...
		result.add(EXTENSION); 
		nlSuffixes = result.toArray(new String[result.size()]);
		if(hashChangePro.containsKey(key)) 
			hashChangePro.remove(key);
	}
	...
	protected static ReflectUtil ref = ReflectUtil.getInstance();
	protected String getBunldeName() {return BUNDLE_NAME;};
	private String localestr = ""; 
	public String msg(String prefix, String key,String...def) { 
		if(null != AbstractBaseAuthorization.get()){ 
			Object l = AbstractBaseAuthorization.get().getSession().getAttribute("language");
			if(null != l&&!localestr.equals(l.toString())){
				localestr = l.toString();
				UNLS.initializeMessages(getBunldeName(), this.getClass()); 
				UNLS.initializeMessages(BUNDLE_NAME, UNLS.class); 
			}
		} 
		if (!StringUtil.isValid(key)) {
			if(null != def&&def.length>0)
				return def[0];
			return key;
		}
		String fullkey = prefix + "_" + key;
		Field f = ref.getField(this.getClass(), fullkey);
		Object o = null;
		if (f != null) {
			o = ref.getFieldValue(this, f);
			if(null != o)
				return o.toString(); 
		}  
		f = ref.getField(this.getClass(), key);
		if (f == null) {
			if(null != def&&def.length>0)
				return def[0];
			return key;
